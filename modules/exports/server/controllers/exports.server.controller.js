...
exports.templateRender = function (req, res) {
 ...
 query.where('_id').equals(req.params.id).lean().populate({
       path: 'workface',
       populate: {
         path: 'workstatusid',
         model: 'Workstatus'
    ...
    if (err) {
      ...
      if(article.price == '') {
        ...
        workstatus = article.workface.workstatusid.workStatus;
        price = parseInt(article.workface.workstatusid.price)+parseInt(article.price);
      }
...
exports.exportContactsToCSV = function (req, res) {
  ...
  //query.populate('workface');
  //query.populate('price');
      ...
      if (err) {
        ...
        for(var k in article){
          //console.log(article[k]);
            var price = "";
            var workstatus = "";
            if(article[k].price != undefined && article[k].workface != undefined && article[k].workface.price != undefined) {

                price = parseInt(article[k].workface.price)+parseInt(article[k].price);
            ...
            if(article[k].workface != undefined && article[k].workface.workstatusid != undefined && article[k].workface.workstatusid.workStatus != undefined) {
                workstatus = article[k].workface.workstatusid.workStatus;
                console.log(workstatus + "testworkstatus");
            } else {
                workstatus = "nil";
            }
          ...
          //console.log("hi we are testin" + article[k].LastnameContact.value);
...
exports.CalculatorToCSV = function (req, res) {
  ...
  var titles = [
       "Full Name", "Claim","Date Assigned","Date Close", "RTW (trans)","TWA End", "RTW (reg/perm)", "TD Daily rate", "Days Saved", "Voucher", "Savings to Date", "Status", "Work Status", "Work Price"
  ];
      ...
      for(var k in article){
        ...
        //console.log(article[k]);
          ...
          if(article[k].price != undefined && article[k].workface != undefined && article[k].workface.price != undefined) {
              ...
              price = parseInt(article[k].workface.price)+parseInt(article[k].price);
              //console.log(price + "finalprice 789");
          } else {
              price = "nil";
          }
          ...
          if(article[k].workface != undefined && article[k].workface.workstatusid != undefined && article[k].workface.workstatusid.workStatus != undefined) {
               workstatus = article[k].workface.workstatusid.workStatus;
               //console.log(workstatus + "finalprice 789");
          } else {
              workstatus = "nil";
          }
         ...
         //   continue;
          ...
          //console.log(workstatus + "finalprice testing 123");

          //console.log(price + "price testing 123");
