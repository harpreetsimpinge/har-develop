...
exports.list = function (req, res) {
  ...
  if(req.body.closed === "open"){
    ...
    //orArr.push({
      //$or:
       // [{'DateClosedFocusInformation.value':null}]
    //});
      query.or([{'DateClosedFocusInformation.value':null}]);

  }
  else if(req.body.closed === "closed"){
    //orArr.push({
      //$or: [{'DateClosedFocusInformation.value': {$gt : new Date(1970, 1, 1)}}]
    //});
      var jsonCloseOR = {};
      jsonCloseOR['DateClosedFocusInformation.value'] = {};
      jsonCloseOR['DateClosedFocusInformation.value']['$gt'] = new Date(1970, 1, 1);
      query.or([jsonCloseOR]);
  ...
  //console.log("Admin1 = "+req.user.roles.indexOf("admin"));
  //console.log("admin2 = "+req.user.roles.indexOf("admin2"));
...
exports.smartSearch = function (req, res) {
	...
	for(var k in smartList)
	{
		...
		var optionCondition = smartList[k].optionCondition;
		
		if(optionCondition == 'or' || optionCondition == ''){
			var re = new RegExp(value, 'i');
			var jsonOR = {};
			if(smartList[k].type === "text" || smartList[k].type === "dropdown"){
				if(smartList[k].option === "contains"){
					jsonOR[field+'.value'] = new RegExp(value,'i');								
				}
				...
				if(smartList[k].option !== "range"){
					...
					{
						jsonOR[field+'.value'] = {};
						jsonOR[field+'.value']['$lt'] = date;				
						jsonOR[field+'.value']['$ne'] = null;				
						jsonOR[field+'.value']['$ne'] = "";				
						jsonOR[field+'.value']['$ne'] = " ";
					}
					else if(smartList[k].option === "exactly")
					{
						...
						d2.setDate(d2.getDate()+1);
						jsonOR[field+'.value'] = {};
						jsonOR[field+'.value']['$gt'] = d1;		
						jsonOR[field+'.value']['$lt'] = d2;
					}
					else if(smartList[k].option === "later")
					{	
						jsonOR[field+'.value'] = {};
						jsonOR[field+'.value']['$gt'] = date.toISOString();								
					}
				}
					...
					jsonOR[field+'.value'] = {};
					jsonOR[field+'.value']['$gt'] = start_date;		
					jsonOR[field+'.value']['$lt'] = end_date;					
				}
				query.or([jsonOR]);		
			}		
		}	
		...
		// AND CLAUSE
			...
			if(field === "closed")
			{
				...
				if(value === "open")
				{	
					//jsonOR['DateClosedFocusInformation.value'] = {};
					//jsonOR['DateClosedFocusInformation.value']['$lte'] = new Date(1970, 1, 2);		
					//jsonOR['DateClosedFocusInformation.value']['$eq'] = "";	
				...
				{
					//jsonOR['DateClosedFocusInformation.value'] = {};
					//jsonOR['DateClosedFocusInformation.value']['$gte'] = new Date();		
			...
			{
				if(smartList[k].option === "contains"){
					//console.log("value"+value+" option"+smartList[k].option);
